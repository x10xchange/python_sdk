...
from x10.errors import SdkNotImplementedError, SdkValidationError
...
def __create_order_object(
    *,
    ...
    def validate_market_order():
        if post_only:
            raise SdkValidationError("MARKET orders must not be post-only")

        if time_in_force != TimeInForce.IOC:
            raise SdkValidationError("MARKET orders must have `time_in_force` set to IOC")

    def validate_conditional_order():
        if not trigger:
            raise SdkValidationError("CONDITIONAL orders must have `trigger` specified")

    def validate_tpsl_order():
        if not reduce_only:
            raise SdkValidationError("TPSL orders must be reduce-only")

        if post_only:
            raise SdkValidationError("TPSL orders must not be post-only")

        if tp_sl_type == OrderTpslType.POSITION and synthetic_amount != Decimal(0):
            raise SdkValidationError("`synthetic_amount` must be 0 for entire position TPSL orders")

        if price != Decimal(0):
            raise SdkValidationError("`price` must be 0 for TPSL orders")

    if order_type not in [OrderType.LIMIT, OrderType.MARKET, OrderType.CONDITIONAL, OrderType.TPSL]:
        raise SdkNotImplementedError(f"{order_type} order type is not supported yet")

    if exact_only:
        raise SdkNotImplementedError("`exact_only` option is not supported yet")

    if time_in_force == TimeInForce.FOK:
        raise SdkValidationError("FOK `time_in_force` value is deprecated")

    if expire_time is None:
        raise SdkValidationError("`expire_time` must be provided")
    ...
    def create_tpsl_trigger_model(trigger_param: OrderTpslTriggerParam | None):
        ...
        if tp_sl_type is None:
            raise SdkValidationError("`tp_sl_type` must be provided if `take_profit` and/or `stop_loss` is specified")
